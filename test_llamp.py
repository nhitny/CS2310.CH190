...
def main():
    ...
    model = LLaMP(
        base_testset, classnames, args, llama_model, tokenizer, few_shot=False
    )
    ...
    for i in range(1, 4):
        try:
            ...
            print(f"Failed to load model from checkpoint {args.load}: {e}")
            return  

        model.load_state_dict(state_dict, strict=False)

        model_engine, _, _, _ = deepspeed.initialize(
            config=deepspeed_config, model=model
        )
        model_engine.eval()
