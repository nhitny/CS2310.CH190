...
#     project_name_suffix = "{num_shots}Shot-{pair_emb}-{lr:.1E}-{lr_scheuler}-{prompt_type}-{num_prior_tokens}Pr{llm_prompt_depth}x{num_llm_prompts}P{num_text_ctx}T{num_vis_ctx}V-{bias}Init-{dist_type}{lambda_dist:.1f}xDist{ema}{randaug}".format(
...
#     model = LlamaForCausalLM.from_pretrained(args.model_base, device_map='auto')
...
#     classnames = {
#         'all': trainset.classnames,
#     }

#     model = LLaMP(trainset, classnames, args, model, tokenizer, few_shot=True)
...
#     optimizer = AdamW(
#         lr=args.lr, weight_decay=args.weight_decay, params=parameter_group
#     )
...
#                 base_lrs=base_lrs,
#                 num_warmup_steps=1,
#                 num_training_steps=args.max_epochs + 1
#             )
#         else:
...
#     evaluator = Classification(args, testset.idx2label)
...
#                 with torch.no_grad():
#                     acc = test(epoch, model_engine, testloader, evaluator, args, logpath, device, subset='all')['accuracy']
#                     print(f'Acc: {acc}')
...
#     model.eval()
#     model.module.compute_all_pair_embeddings(subset=subset.lower())
...
def get_cosine_schedule_lambda_with_warmup(
    ...
    def lr_lambda(current_step):
        if current_step < num_warmup_steps:
            return max(
                min_lr / base_lr, float(current_step) / float(max(1, num_warmup_steps))
            )
...
def get_cosine_schedule_with_warmup(
    ...
    for min_lr, base_lr in zip(min_lrs, base_lrs):
        lr_lambda.append(
            get_cosine_schedule_lambda_with_warmup(
                min_lr, base_lr, num_warmup_steps, num_training_steps, num_cycles
            )
        )
    return LambdaLR(optimizer, lr_lambda, last_epoch)
...
def save_checkpoint(model, epoch, logpath, seed, filename):
    ...
    torch.save(state, os.path.join(logpath, f"ckpt_{filename}_{seed}.t7"))
...
def build_optimizer_parameters(config, model):
    ...
    param_optimizer = [
        ...
        if "pooler" not in n[0]
        and "lora" not in n[0]
        and "visual_projection" not in n[0]
    ]
    lora_params = [
        n
        for n in model.named_parameters()
        if "lora" in n[0] or "visual_projection" in n[0]
    ]
    weight_decay = getattr(config, "weight_decay", 0.01)
    optimizer_grouped_parameters = [
        {
...
def main():
    ...
    project_name_suffix = f"{args.coop_num_shots}Shot-{{:d}}Xdecode-{Decimal(args.lr):.1E}-{args.lr_scheduler or 'constant'}-{args.prompt_type}-{args.num_prior_tokens}Pr{args.llm_prompt_depth}x{args.num_llm_prompts}P{args.num_text_ctx}T{args.num_vis_ctx}V-{'Bias' if args.token_bias else 'No'}Init-{args.distillation_type}{args.lambda_dist:.1f}xDist{'-RandAug' if args.rand_aug else ''}"
    project_name_suffix += "-Skip" if args.decoder_skip_connection else ""
    project_name_suffix += "-ConcatPrior" if args.concat_fixed_prompts else ""
    args.name += project_name_suffix
    ...
    model = LlamaForCausalLM.from_pretrained(args.model_base, torch_dtype=torch.float16)
    ...
    classnames = {"all": trainset.classnames}
    ...
    print("Model on device:", next(model_engine.parameters()).device)
    ...
    for epoch in tqdm(range(start_epoch, args.max_epochs), desc="Current epoch"):
        trainloader.sampler.set_epoch(epoch)
        train(epoch, model_engine, tokenizer, trainloader, False, lr_scheduler, device)
        if is_main_process() and (epoch + 1) % args.eval_val_every == 0:
            with torch.no_grad():
                acc = test(
                    epoch,
                    model_engine,
...
def train(epoch, model_engine, tokenizer, trainloader, ema, lr_scheduler, device):
    model_engine.train()
    train_loss = {"loss_total": 0.0, "loss_ce": 0.0, "loss_l1": 0.0, "loss_dist": 0.0}
    ...
    train_loss = {k: v / len(trainloader) for k, v in train_loss.items()}
    print(f'Epoch: {epoch}| Loss: {round(train_loss["loss_total"], 2)}')
...
def test(epoch, model, testloader, evaluator, args, logpath, device, subset):
    ...
    stats = evaluator.evaluate()
    stats["a_epoch"] = epoch
    result = "".join([f"{key}  {round(stats[key], 4)}| " for key in stats]) + args.name
    print(f"Test Epoch: {epoch}")
    print(result)
    return stats
